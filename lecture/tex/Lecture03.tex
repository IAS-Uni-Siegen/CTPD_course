...
\begin{frame}
    ...
    \begin{equation}
        \bm{\psi}_\mathrm{s,dq}(t)  \approx \underbrace{\bm{f}_{\psi_\mathrm{s}}(\bm{0})}_{\displaystyle=\bm{\psi}_\mathrm{PM}} + \underbrace{\frac{\partial \bm{f}_{\psi_\mathrm{s}}}{\partial \bm{i}_\mathrm{s,dq}}\bigg|_{\bm{i}_\mathrm{s,dq}=\bm{0}}}_{\displaystyle=\,\bm{L}_0}\bm{i}_\mathrm{s,dq}(t)  \onslide<2->{= \begin{bmatrix}
                    \psi_{\mathrm{PM}} \\
                    0
...
\begin{frame}
    ...
    As can be seen from \eqref{fig:inductance_maps_BRUSA} the off-diagonal inductance terms $L_{\mathrm{dq},0}$ and $L_{\mathrm{qd},0}$ are much smaller than the main diagonal terms $L_{\mathrm{dd},0}$ and $L_{\mathrm{qq},0}$. \pause Thus, by further simplification
    $$
...
\begin{frame}
    ...
    \end{equation*}
    pplying the \hl{excplicit Euler method (RK1)} from \eqref{eq:RK1} yields the discrete-time model
    \begin{equation}
    ...
    \end{equation}
    where $T_\mathrm{s}$ is the sampling period and $k$ is the \hl{discrete time index}. Likewise, the \hl{implicit Euler method (IRK1)} from \eqref{eq:IRK1} gives
    \begin{equation}
        \begin{split}
...
\begin{frame}
    ...
    \end{equation*}
    Hence, for any $\omega[k+1]>0$, the IRK1 induced voltage vector is less than $\nicefrac{\pi}{2}$ shifted towards the flux vector.
\end{frame}
...
\begin{frame}
    ...
    \end{equation}
    A few remarks on the above model set:
    \begin{itemize}
        \item Although typically more accurate and numerical robust than a direct RK1 discretization of the $\mathrm{dq}$-frame model, the above RK1 model is still an approximation of the continuous-time dynamics.
        \item Consequently, numerical errors cannot be completely avoided. The severity of the numerical errors depend on the specific machine parameters, operating point and sampling time.
        \item If interested in the stator currents, one can solve \eqref{eq:basic_PMSM_model_set_RK1} and then apply the inverse flux linkage maps $\bm{f}_{\psi_\mathrm{s}}^{-1}(\cdot)$.
    \end{itemize}
\end{frame}
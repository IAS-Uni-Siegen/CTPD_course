...
\begin{frame}
    ...
    \begin{equation}
        \frac{\mathrm{d}}{\mathrm{d}t}\bm{i}_\mathrm{E}(t) = \bm{L}_{\mathrm{E},0}^{-1}\left[\bm{u}_\mathrm{E}(t) - \bm{R}_\mathrm{E}\bm{i}_\mathrm{E}(t) - \bm{J}_\mathrm{E}(\omega)\bm{L}_{\mathrm{E},0}\bm{i}_\mathrm{E}(t)\right].
    \end{equation}\pause
...
\begin{frame}
    ...
    \begin{equation}
        \bm{f}_{\psi_\mathrm{s}}(\bm{i}_\mathrm{s,dq}[k+1]) \approx \bm{f}_{\psi_\mathrm{s}}(\bm{i}_\mathrm{s,dq}[k]) +\bm{L}_\mathrm{s}(\bm{i}_\mathrm{s,dq}[k])\left(\bm{i}_\mathrm{s,dq}[k+1] - \bm{i}_\mathrm{s,dq}[k]\right),
    \end{equation}
    ...
    \begin{equation}
        \begin{split}
            \bm{i}_\mathrm{s,dq}[k+1]  = & \underbrace{\left(\bm{I} - \bm{L}^{-1}_\mathrm{s}(\bm{i}_\mathrm{s,dq}[k]) \bm{T}_\mathrm{p}(-\omega[k]T_\mathrm{s})T_\mathrm{s}R_\mathrm{s}\right)\bm{i}_\mathrm{s,dq}[k]}_{\text{discrete-time PT1 element}}  \\
                                         & + \bm{L}_\mathrm{s}^{-1}(\bm{i}_\mathrm{s,dq}[k])\underbrace{\left(\vphantom{\bm{L}_\mathrm{s}^{-1}}\bm{T}_\mathrm{p}(-\omega[k]T_\mathrm{s})-\bm{I}\right)\bm{\psi}_{\mathrm{dq}}[k]}_{\text{induced voltage}} \\
                                         & +\left( \bm{L}_\mathrm{s}^{-1}(\bm{i}_\mathrm{s,dq}[k])\bm{T}_\mathrm{p}(-\omega[k]T_\mathrm{s}) T_\mathrm{s}\right)\underbrace{\bm{u}_{\mathrm{dq}}[k]}_{\text{input voltage}}.
        \end{split}
...
\begin{frame}
    ...
    $$
        ...
        \end{bmatrix}\bm{i}_\mathrm{s,dq} + \begin{bmatrix}
            \psi_\mathrm{PM} \\
            0
        \end{bmatrix} = \bm{L}_0\bm{i}_\mathrm{s,dq} + \bm{\psi}_\mathrm{PM}.
    $$
    This was already the case in \eqref{eq:PMSM_current_ODE_linear_mdl} from which we can derive the continous-time system matrix $\bm{A}_\mathrm{c}$ assuming a constant speed $\omega(t) = \omega$
    \begin{equation}
        \bm{A}_\mathrm{c}  = \bm{L}_0^{-1}\left(-R_\mathrm{s}\bm{I}-\omega\bm{J}\bm{L}_0\right) = \begin{bmatrix}
            -\frac{R_\mathrm{s}}{L_{\mathrm{d}}}         & -\omega\frac{L_{\mathrm{q}}}{L_{\mathrm{d}}} \\[1ex]
            -\omega\frac{L_{\mathrm{d}}}{L_{\mathrm{q}}} & -\frac{R_\mathrm{s}}{L_{\mathrm{q}}}
        \end{bmatrix}.
    \end{equation}
    The eigenvalues of this continuous-time system matrix are given by
    \begin{equation}
        \lambda_{1,2} = -\frac{R_\mathrm{s}}{2}\left(\frac{1}{L_{\mathrm{d}}}+\frac{1}{L_{\mathrm{q}}}\right) \pm \frac{1}{2}\sqrt{\left(\frac{R_\mathrm{s}}{L_{\mathrm{d}}}-\frac{R_\mathrm{s}}{L_{\mathrm{q}}}\right)^2 - 4\omega^2}.
    \end{equation}
    Since the real part of the eigenvalues is always negative for physically reasonable assumptions $\left\{L_{\mathrm{d}}, L_{\mathrm{q}}, R_\mathrm{s}, \omega \right\}> 0$, \hl{the continuous-time system is stable}.
\end{frame}
...
\begin{frame}
    ...
    Applying the RK1 discretization to the linear current ODE \eqref{eq:linear_current_ODE_PMSM} in the $\mathrm{dq}$-frame yields the following current difference equation:
    \begin{equation}
        \begin{split}
            \bm{i}_\mathrm{s,dq}[k+1] = \left(\bm{I} - \bm{L}_0^{-1}R_\mathrm{s}-\omega T_\mathrm{s}\bm{J}\right)\bm{i}_\mathrm{s,dq}[k] + \bm{L}_0^{-1}\bm{u}_\mathrm{dq}[k] - \bm{L}_0^{-1}\omega T_\mathrm{s}\bm{J}\bm{\psi}_\mathrm{PM}.
        \end{split}
    \end{equation}
    In this case, the system matrix is
    \begin{equation}
        \bm{A}_{\text{RK1}} = \bm{I} - \bm{L}_0^{-1}R_\mathrm{s}-\omega T_\mathrm{s}\bm{J} = \begin{bmatrix}
            1-\frac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{d}}}         & -\omega T_\mathrm{s}\frac{L_{\mathrm{q}}}{L_{\mathrm{d}}} \\[1ex]
            -\omega T_\mathrm{s}\frac{L_{\mathrm{d}}}{L_{\mathrm{q}}} & 1-\frac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{q}}}
        \end{bmatrix}.
    \end{equation}
    The resulting eigenvalues of the system matrix are given by
    \begin{equation}
        \lambda_{1,2} = 1 - \frac{T_\mathrm{s}R_\mathrm{s}}{2}\left(\frac{1}{L_{\mathrm{d}}}+\frac{1}{L_{\mathrm{q}}}\right) \pm \frac{T_\mathrm{s}}{2}\sqrt{\left(\frac{R_\mathrm{s}}{L_{\mathrm{d}}}-\frac{R_\mathrm{s}}{L_{\mathrm{q}}}\right)^2 - 4\omega^2}.
    \end{equation}
    The eigenvalues are in the unit circle (i.e., the discrete-time system is stable) if
    \begin{equation}
        \|\lambda_{1,2}\| < 1 \quad \Leftrightarrow \quad T_\mathrm{s} < \frac{R_\mathrm{s}\left(L_\mathrm{d} + L_\mathrm{q}\right)}{R_\mathrm{s}^2 + \omega^2 L_\mathrm{d}L_\mathrm{q}}.
...
\begin{frame}
    ...
    $$
        L_\mathrm{d} = \SI{0.5}{\milli\henry}, \quad L_\mathrm{q} = \SI{1}{\milli\henry}, \quad R_\mathrm{s} = \SI{0.01}{\ohm}, \quad \omega = \SI{2\pi \cdot \frac{1000}{60}\cdot10}{\radian\per\second}.
    $$
...
\begin{frame}
    ...
    \begin{equation}
        \begin{split}
            \bm{i}_\mathrm{s,dq}[k+1] = & \left(\bm{I} - \bm{L}_0^{-1}\bm{T}_\mathrm{p}(-\omega T_\mathrm{s})R_\mathrm{s}T_\mathrm{s}\right)\bm{i}_\mathrm{s,dq}[k] \\ &+ \bm{L}_0^{-1}\left(\bm{T}_\mathrm{p}(-\omega T_\mathrm{s})-\bm{I}\right)\left(\bm{L}_0\bm{i}_\mathrm{s,dq}[k] + \bm{\psi}_\mathrm{PM}\right) + \bm{L}_0^{-1}\bm{T}_\mathrm{p}(-\omega T_\mathrm{s}) T_\mathrm{s}\bm{u}_{\mathrm{dq}}[k]\\
            =                           & \bm{T}_\mathrm{p}(-\omega T_\mathrm{s})\left(\bm{I} - \bm{L}_0^{-1}R_\mathrm{s}T_\mathrm{s}\right)\bm{i}_\mathrm{s,dq}[k] \\ &+ \bm{L}_0^{-1}\left(\bm{T}_\mathrm{p}(-\omega T_\mathrm{s})-\bm{I}\right)\bm{\psi}_\mathrm{PM} + \bm{L}_0^{-1}\bm{T}_\mathrm{p}(-\omega T_\mathrm{s}) T_\mathrm{s}\bm{u}_{\mathrm{dq}}[k].
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        \bm{A}_{\text{RK1}}^{\upalpha\upbeta\rightarrow\mathrm{dq}} = \begin{bmatrix}
            ...
            \left(1-\dfrac{T_\mathrm{s} R_\mathrm{s}}{L_{\mathrm{d}}}\right)
             &
            \sin\!\left(\omega T_\mathrm{s}\right)
            \dfrac{L_{\mathrm{q}}}{L_{\mathrm{d}}}
            \left(1-\dfrac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{q}}}\right)
            \\[2ex]
            -\sin\!\left(\omega T_\mathrm{s}\right)
            \dfrac{L_{\mathrm{d}}}{L_{\mathrm{q}}}
            \left(1-\dfrac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{d}}}\right)
             &
            \cos\!\left(\omega T_\mathrm{s}\right)
            \left(1-\dfrac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{q}}}\right)
    ...
    \begin{equation}
        \begin{split}
            ...
            \sqrt{
                ...
                4
                \left(
                1-\frac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{d}}}
                \right)
                \left(
                1-\frac{T_\mathrm{s}R_\mathrm{s}}{L_{\mathrm{q}}}
                \right)
            }
        \end{split}
    \end{equation}
    For $\left\{L_{\mathrm{d}}, L_{\mathrm{q}}, R_\mathrm{s}, \omega \right\}> 0$ it can be shown that
    \begin{equation}
        \|\lambda_{1,2}\| < 1 \quad \Leftrightarrow \quad T_\mathrm{s} < 2\frac{\min\left\{L_\mathrm{d}, L_\mathrm{q}\right\}}{R_\mathrm{s}}
    \end{equation}
...
\begin{frame}
    ...
    \begin{equation}
        \begin{alignedat}{4}
             ...
             &  & \mbox{Flux linkage:}   &  & \quad \bm{\psi}_\mathrm{€}[ & k+1]    &  & = \bm{f}_{\psi, \mathrm{E}}(\bm{i}_\mathrm{E}[k+1]),                                                                                                                     \\
             &  & \mbox{Torque:}         &  & \quad T[                       & k+1] &  & = \nicefrac{3}{2} p (\bm{i}_\mathrm{s,dq}[k+1])\T\bm{J}\bm{\psi}_\mathrm{s,dq}[k+1].
        \end{alignedat}
    \end{equation}
\end{frame}

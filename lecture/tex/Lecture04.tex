...
\begin{frame}
    ...
    \begin{figure}
        ...
        \begin{circuitikz}[straight voltages]
            ...
            % lower return path with rotational voltage sources
            \draw (LB) to [V, v_<=$\omega_{k}\bm{J}\bm{\psi}^{\mathrm{k}}_{\mathrm{s,dq}}$] (C |- LB);
            \draw (N |- LB)  to [V, v_=$(\omega_{\mathrm{k}}-\omega)\bm{J}\bm{\psi}^{\mathrm{k}}_{\mathrm{r,dq}}$] (RB |- LB);
            \draw (N |- LB) to (C |- LB);
            ...
            % magnetizing branch
            \draw
            (C) to[L, l_=$L_{\mathrm{m}}$, i>_=$\bm{i}^{\mathrm{k}}_{\mathrm{m,dq}}$, *-*] (C |- LB);
            \draw
            (N) to[R, l=$R_{\mathrm{c}}$, i>^=$\bm{i}^{\mathrm{k}}_{\mathrm{c,dq}}$] (N |- LB);
            ...
            % flux linkage voltage references
            \draw (3.2,2.5) to[open, v=$\frac{\mathrm{d}}{\mathrm{d}t}\bm{\psi}^{\mathrm{k}}_{\mathrm{s,dq}}$] (3.2,0);
            \draw (7.8,2.5) to[open, v^>=$\frac{\mathrm{d}}{\mathrm{d}t}\bm{\psi}^{\mathrm{k}}_{\mathrm{r,dq}}$] (7.8,0);
        \end{circuitikz}
        \caption{Extended T-type equivalent model with additional iron loss resistance $R_\mathrm{c}$ in the magnetizing branch}
        \label{fig:T_ECD_IM_dq_iron_loss}
    \end{figure}
\end{frame}
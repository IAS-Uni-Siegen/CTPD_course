...
\begin{frame}
    ...
    \end{equation}\pause%
    As in the PMSM case, all controller-relevant variables are evaluated after the one-step prediction that compensates the digital control delay.
\end{frame}
...
\begin{frame}
    ...
    We seek a feedforward decoupling structure that transforms \eqref{eq:IM_current_control_qLPV_plant} into the diagonal first-order model
    \begin{equation}
...
\begin{frame}
    ...
    \end{equation}\pause%
    Therefore, the modified stator-voltage command is
    \begin{equation}
        \begin{split}
            \bm{u}^{*}[k]
             & =-\bm{u}^{\psi_\mathrm{r}}_{\mathrm{rot},\mathrm{dq}}[k]
...
\begin{frame}
    ...
    \begin{figure}
        ...
        \end{circuitikz}%
        \caption{Rotor flux and torque controller representation with ARW feedback}
...
\begin{frame}
    ...
    The current controller addendum provided two decoupled stator-current loops in the rotor-flux-oriented frame. For the flux-control design, the inner $d$-axis current loop is therefore approximated by its closed-loop reference response:
    \begin{equation}
    ...
    \end{equation}\pause%
    Here, $\lambda_\mathrm{i}$ is the remaining current-loop pole after the pole-zero cancellation used in the SISO current-loop design. The outer rotor-flux controller generates
    \begin{equation}
        i^{*}_{\mathrm{s,d}}[k]
...
\begin{frame}
    ...
    \end{equation}\pause%
    For $\lambda_\mathrm{i}\ll a_\psi[k]$ in the sense of time-scale separation, the fast pole is commonly neglected for the outer-loop tuning:
    \begin{equation}
...
\begin{frame}
    ...
    \end{equation}\pause%
    Using the plant in \eqref{eq:IM_flux_plant_with_current_loop}, the closed-loop reference transfer function is
    \begin{equation}
...
\begin{frame}
    ...
    For the usual bandwidth separation $\lambda_\mathrm{i}\ll p_1,p_2\approx a_\psi$, the inner current loop can be approximated by unity for the flux-loop tuning. Then
    \begin{equation}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item For fast field build-up, reserve $d$-axis current margin and test interaction with $i^{*}_{\mathrm{s,q}}$.
...
\begin{frame}
    ...
    \end{equation}\pause%
    Thus, the open-loop torque controller maps the reference torque to the torque-producing current reference
    \begin{equation}
        i^{*}_{\mathrm{s,q}}[k]
...
\begin{frame}
    ...
    \begin{equation}
        \tilde{\bm{i}}^{*}_{\mathrm{s,dq}}[k]
        =\begin{bmatrix}
            \tilde{i}^{*}_{\mathrm{s,d}}[k] \\
            \tilde{i}^{*}_{\mathrm{s,q}}[k]
        \end{bmatrix} .
    \end{equation}\pause%
    With a simple magnitude clipping strategy, the orientation of this reference vector is kept unchanged:
    \begin{equation}
        \gamma[k]
        =\min\left\{1,\frac{i_{\mathrm{s},\max}}{\left\|\tilde{\bm{i}}^{*}_{\mathrm{s,dq}}[k]\right\|_2}\right\},
        \qquad
        \bm{i}^{*}_{\mathrm{s,dq}}[k]
        =\gamma[k]\tilde{\bm{i}}^{*}_{\mathrm{s,dq}}[k].
        \label{eq:IM_flux_mutual_current_limit}
    \end{equation}\pause%
    Consequences:
    \begin{itemize}
        \item $\gamma=1$: flux and torque commands are feasible.
        \item $\gamma<1$: both axes are reduced proportionally.
        \item The implemented current vector is feasible, but flux and torque references cannot both be fulfilled.
...
\begin{frame}
    ...
    \begin{equation}
        i^{*}_{\mathrm{s,d}}[k]=\gamma[k]\tilde{i}^{*}_{\mathrm{s,d}}[k],
        \qquad
        \Delta i_\psi[k]=\tilde{i}^{*}_{\mathrm{s,d}}[k]-i^{*}_{\mathrm{s,d}}[k].

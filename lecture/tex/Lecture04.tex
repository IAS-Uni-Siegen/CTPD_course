...
\begin{frame}
    ...
    \end{equation}\pause%
    \begin{itemize}
        \item The rotor flux reference $\psi^*_\mathrm{lim}$ decides if operating in the loss-optimal constant torque region (low to medium speeds) or if flux weakening is required (medium to high speeds).
        \item In the flux weakening region, the torque reference $T^*_\mathrm{lim}$ may be reduced below $T^*$ in order to satisfy the voltage and current limits.
    \end{itemize}\vspace{-0.25cm}
    \begin{varblock}{Similarities and differences to the PMSM OPS}
        Likewise in the PMSM case, the IM's OPS is a static mapping problem considering steady-state conditions such that a given torque reference is mapped to a feasible and efficient operating point. However, we do not map directly into the stator current plane (like with the PMSM) due to the additional presence of rotor flux dynamics.
    \end{varblock}
\end{frame}
...
\begin{frame}
    ...
    \end{equation}
    Hence, the torque can be expressed as
    \begin{equation}
        T
    ...
    \end{equation}\pause%
    The copper-loss model contains stator and rotor contributions:
    \begin{equation}
...
\begin{frame}
    ...
    From \eqref{eq:basic_IM_model_set} the IM's steady-state stator voltage demand is
    \begin{equation}
    ...
    \end{equation}
    Here, the ohmic voltage drop is considered negligible compared to the rotatory induced voltage. With \eqref{eq:stator_flux_IM_stray_constant} and \eqref{eq:steady-state_rotor_flux_IM_id} we express the stator flux as a function of the stator currents:
    \begin{equation}
        \bm{\psi}_\mathrm{s,dq} = \begin{bmatrix}
            L_\mathrm{s,app} i_\mathrm{s,d} \\ \sigma L_\mathrm{s,app} i_\mathrm{s,q}
        \end{bmatrix}.
    \end{equation}
    Hence, the squared steady-state stator voltage magnitude becomes
    \begin{equation}
...
\begin{frame}
    ...
    For a given objective $J_{\mathrm{OPS}}$, the OPS problem reads
    \begin{equation}
        \begin{aligned}
            \bm{i}_{\mathrm{s,dq}}^{*}
             & \in
    ...
    \end{equation}\pause%
    The relevant objective functions are
    \begin{equation}
        \begin{aligned}
            J_{\mathrm{MTPC}} & =\left\|\bm{i}_{\mathrm{s,dq}}\right\|_2^2,
                              & \quad                                          & \text{maximum torque per (stator) current},             \\
            J_{\mathrm{MTPL}} & =P_{\mathrm{Cu}},
                              &                                                & \text{maximum torque per (stator/rotor copper) losses}, \\
...
\begin{frame}
    ...
    We define the relevant contours in the rotor-flux-oriented stator current plane ($\bm{i}=\bm{i}_{\mathrm{s,dq}}$):
    \vspace{-0.2cm}
        ...
        \begin{column}{0.475\textwidth}
            \begin{itemize}
                ...
                \item<2-> Visual interpretation: tangent point of the torque contour and the copper-loss contour:
                      $$
                          \partial T(\bm{i}_{\mathrm{s,dq}}^{\psi_{\mathrm r}})=\lambda\,\partial P_{\mathrm{Cu}}(\bm{i}_{\mathrm{s,dq}}^{\psi_{\mathrm r}}).
                      $$
    ...
    \begin{columns}
        ...
        \begin{column}{0.475\textwidth}
            \begin{itemize}
                ...
                \item<2-> The OPS can no longer follow the MTPL or MTPC loci due tohe voltage limit.
    ...
    \begin{columns}
        \begin{column}{0.55\textwidth}
            \begin{itemize}
                ...
                \item Since the stator current is limited, the closed-loop rotor flux dynamics become dominant in the torque response.
                \item Compared to an ``always nominal flux'' strategy, the loss-optimal rotor-flux selection can lead to a slower torque response.
            \end{itemize}
        \end{column}
        \begin{column}{0.35\textwidth}
            \begin{figure}
...
\begin{frame}
    ...
    \begin{figure}
        ...
        \end{circuitikz}%
        \caption{Modulation degree controller  (inner PI + ARW loop is structural identical to PMSM case from \figref{fig:modulation_degree_control} and is not shown here due to space reasons).}
...
\begin{frame}
    ...
    \begin{equation}
        m[k]=\frac{\sqrt{3}}{2}\frac{\left\|\bm{u}^{\psi_{\mathrm r},*}_{\mathrm{s,dq}}[k]\right\|_2}{u_{\mathrm{dc}}[k]},
        \qquad e_m[k]=m^*[k]-m[k].
    ...
    \begin{equation}
        ...
        \qquad
        \psi'_{\mathrm{s,max}}=\frac{2u_{\mathrm{dc}}}{\sqrt{3}|\omega_{\psi_{\mathrm r}}|} .
        \label{eq:IM_mod_v06_classical_stator_limit}
    \end{equation}\pause%
    In an IM rotor-flux-oriented FOC, however, the OPS and flux controller do not manipulate $\psi_{\mathrm{s}}$ directly.  Their controlled variable is
    \begin{equation}
...
\begin{frame}
    ...
    \begin{equation}
        ...
        \qquad
        K_m=\frac{\sqrt{3}|\omega_{\psi_{\mathrm r}}|}{2u_{\mathrm{dc}}} .
...
\begin{frame}
    ...
    The local small-signal chain based on the controller structure from \figref{fig:modulation_degree_control_IM} is
    \begin{equation}
...
\begin{frame}
    ...
    \]
    For reduced modulation-controller design, approximate it by a unity-gain PT1 model
    \begin{equation}
        G'_{\psi}(z)
        =
...
\begin{frame}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \approx K_mG'_{\psi}(\mathrm{z}),                      \\[1mm]
...
\begin{frame}
    ...
    The discrete-time PI controller now outputs a rotor-flux-limit correction:
    \begin{equation}
    ...
    \end{equation}
    It is the same PT1-plant design problem as before, but with a different plant gain.
\end{frame}
...
\begin{frame}
    ...
    \begin{equation}
        b_m=\frac{\sqrt{3}|\omega_{\psi_{\mathrm r}}|}{2u_{\mathrm{dc}}}
...
\begin{frame}
    ...
    \begin{figure}
        ...
        \end{circuitikz}
        \caption{Overview of a direct torque control system for a IM drives (not considering superimposed OPS and modulation level control levels, the DTC for PMSM and IM are structurally identical).}
...
\begin{frame}
    ...
    \end{equation}\pause%
    The IM's torque can be represented as the dot product of the stator and rotor flux linkages:
    \begin{equation}
        \begin{split}
    ...
    \end{equation}\pause%
    \begin{itemize}
        \item The rotor flux linkage is a comparatively slow state due to the rotor time constant.
...
\begin{frame}
    ...
    \end{equation}\pause%
    A simple discrete-time voltage-model estimator is
    \begin{equation}
    ...
    \end{equation}\pause%
    \begin{itemize}
        \item $\hat{\bm{u}}_\mathrm{s}$ is typically reconstructed from $s_\mathrm{a,b,c}$, $u_\mathrm{dc}$ and an inverter model.
...
\begin{frame}
    ...
    Substituting \eqref{eq:IM_DTC_apparent_current_elimination_v6} into \eqref{eq:IM_DTC_flux_odes_stator_frame_v6} yields a coupled ODE system with only flux linkages as states:
    \begin{equation}
        \begin{aligned}
            \frac{\mathrm{d}}{\mathrm{d}t}\hat{\bm{\psi}}^{\mathrm{s}}_{\mathrm{s},\upalpha\upbeta}
             & =\hat{\bm{u}}^{\mathrm{s}}_{\mathrm{s},\upalpha\upbeta}
...
\begin{frame}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \hat{\omega}_{\psi_\mathrm{r}}[k]
             & =\omega[k]+\frac{L_\mathrm{m,app}}{\tau_\mathrm{r}\hat{\psi}^{\psi_\mathrm{r}}_{\mathrm{r,d}}[k]}i^{\psi_\mathrm{r}}_{\mathrm{s,q}}[k], \\
    ...
    \end{equation}\pause%
    This route avoids integrating the voltage $\bm{u}_\mathrm{s}-R_\mathrm{s}\bm{i}_\mathrm{s}$, but it needs rotor-speed information and accurate rotor parameters. Moreover, it requires a more coordinate transformations.
\end{frame}
        ...
        \begin{column}{0.50\textwidth}
            \begin{tcolorbox}[enhanced,arc=1mm,boxrule=0pt,frame hidden,drop fuzzy shadow=black!70,colback=gray!12,colbacktitle=defaultcolor,title={Implementation remarks},fonttitle=\normalsize,left=1mm,right=1mm,top=1mm,bottom=1mm]
                \begin{itemize}
                    ...
                    \item Consider the current model at low speed, but validate rotor-parameter sensitivity.
                    \item Adapt hysteresis widths to control the average switching frequency.
                    \item Choose $\|\bm{\psi}_\mathrm{s}\|^*$ from the OPS mapping: $\|\bm{\psi}_\mathrm{r}\|^* \rightarrow\|\bm{\psi}_\mathrm{s}\|^*$
                \end{itemize}
            \end{tcolorbox}
        \end{column}
        \begin{column}{0.49\textwidth}

...
\begin{frame}
    ...
    \begin{equation}
        ...
        \begin{array}{l}
            \bm{x}(0)=\bm{x}_0,\quad
            \frac{\mathrm{d}}{\mathrm{d}t}\bm{x}(t)=\bm{f}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\hat{\bm{x}}(t),\bm{r}(t)),\bm{w}(t)), \\
...
\begin{frame}
    ...
    \end{equation*}
    and $\bm{u}_{\mathrm{x},\mathrm{dq}}$ denotes an external induced-voltage contribution.\pause%
    With
    \begin{equation*}
...
\begin{frame}
    ...
    \begin{itemize}
        \item simple lower bound: $\underline{\mathcal{J}}_{m}^{\mathrm{tail}}=0$,\pause%
...
\begin{frame}
    ...
    \begin{equation*}
        \begin{array}{rcl}
            \text{short horizons}             & : & T_\mathrm{s}\text{ is much smaller than in modulator-based CCM},     \\[1mm]\pause%
...
\begin{frame}
    ...
    \end{equation}\pause%
    The \hl{discount factor} $\gamma$ weights future rewards: small $\gamma$ emphasizes immediate behavior, while $\gamma\rightarrow 1$ emphasizes long-term consequences.\pause%
    ...
    \begin{equation}
        \boxed{
            ...
            =\mathbb{E}\left[
                ...
                \right].}
    \end{equation}\pause%
    Here, $(\bm{z}',\bm{u}')$ are the next state and action (at time $k+1$), respectively.\pause% 
    For a \hl{parameterized approximation} $q_{\bm{w}}\approx q^{*}$, the temporal-difference target is
    \begin{equation}
        y=r_{\mathrm{RL}}[k+1]
        +\gamma\max_{\bm{u}'}
...
\begin{frame}
    ...
    \begin{equation}
        ...
        q_{\bm{w}}(\bm{z},\bm{u})
    \end{equation}
    at every sampling instant.\pause%
    \hl{Actor-critic methods} avoid this inner optimization by learning a \hl{direct / explicit mapping} from state to actions:
    \begin{equation*}
...
\begin{frame}
    ...
    \begin{block}{Key idea}
        If model, policy, costs, and soft constraint penalties are differentiable, the finite-horizon closed-loop loss can be minimized by gradient-based learning (i.e., supervised learning).
    \end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle{DÜC: high-level visualization}
    \begin{figure}

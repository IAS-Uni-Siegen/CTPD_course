...
\begin{frame}
	...
	\begin{equation}
		...
		\begin{array}{l}
			\bm{x}(0)=\bm{x}_0,\quad
			\frac{\mathrm{d}}{\mathrm{d}t}\bm{x}(t)=\bm{f}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\hat{\bm{x}}(t),\bm{r}(t))), \\
			\bm{g}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\hat{\bm{x}}(t),\bm{r}(t)))\leq\bm{0},                               \\
			\bm{x}(t)\in\mathcal{X},\quad
...
\begin{frame}
	...
	For a current reference $\bm{i}_{\mathrm{s,dq}}^{\ast}$ and voltage deviation $\Delta u = \bm{u}_{\mathrm{s,dq}} - \bm{u}_{\mathrm{s,dq}}^*$ with $\bm{u}_{\mathrm{s,dq}}^*$ being the equilibrium voltage in the requested current set point, define
	\begin{equation*}

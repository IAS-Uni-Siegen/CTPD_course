...
\begin{frame}
    ...
    \begin{center}
        \begin{tabular}{lll}
            \textbf{MPC} & $\rightarrow$ & solve a finite-horizon OCP online at each sampling step,            \\[1mm]
            \textbf{RL}  & $\rightarrow$ & learns an explicit policy approximation from reward samples,        \\[1mm]
            \textbf{DPC} & $\rightarrow$ & train a parametrized policy through a differentiable rollout model.
        \end{tabular}
    \end{center}
    ...
    \end{equation*}
    offline over a distribution of operating scenarios $\bm{\xi}\sim\mathcal{D}$.
...
\begin{frame}
    ...
    \end{equation*}
    can be computed by automatic differentiation.
    \begin{block}{Interpretation}
...
\begin{frame}
    ...
    \end{equation*}
    Equivalently, modern implementations compute the same gradient by reverse-mode automatic differentiation.

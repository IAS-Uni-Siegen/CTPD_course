...
\begin{frame}
	...
	\begin{equation}
		\mathcal{J}_\infty(\bm{x}_0;\bm{\pi})=
		\int_{0}^{\infty}
		\ell(\bm{x}(t),\bm{\pi}(\bm{x}(t),\bm{r}(t)),\bm{r}(t))\,\mathrm{d}t
	...
	\begin{equation}
		\begin{gathered}
			...
			\bm{f}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\bm{x}(t),\bm{r}(t))),\\
			\bm{g}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\bm{x}(t),\bm{r}(t)))\leq\bm{0},\quad
			\bm{x}(t)\in\mathcal{X},\quad
			\bm{\pi}(\bm{x}(t),\bm{r}(t))\in\mathcal{U}.
		\end{gathered}
	\end{equation}
\end{frame}
...
\begin{frame}
	...
	\begin{equation}
		...
		\begin{array}{l}
			\bm{x}(0)=\bm{x}_0,\quad
			\frac{\mathrm{d}}{\mathrm{d}t}\bm{x}(t)=\bm{f}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\bm{x}(t),\bm{r}(t))), \\
			\bm{g}_\mathrm{c}(\bm{x}(t),\bm{\pi}(\bm{x}(t),\bm{r}(t)))\leq\bm{0},                               \\
			\bm{x}(t)\in\mathcal{X},\quad
			\bm{\pi}(\bm{x}(t),\bm{r}(t))\in\mathcal{U},\quad t\geq0
		\end{array}
		\right\} .
		\label{eq:admissible_policy_set}
	\end{equation}\pause\vspace{-0.3cm}
	\begin{varblock}{Interpretation}
		The plant model, constraints and performance index define the controller framework. Classical model-based and upcoming learning-based methods differ mainly in how they solve the OCP. Because an exact solution is usually intractable, it is approximated by one of the methods discussed in the following sections.
	\end{varblock}
\end{frame}
...
\begin{frame}
	...
	For the nominal case $\nicefrac{\mathrm{d}\bm{x}}{\mathrm{d}t}=\bm{f}_\mathrm{c}(\bm{x},\bm{u})$, define the \hl{optimal value function}
	\begin{equation}
		v^*(\bm{x})=
		\inf_{\bm{\pi}}
		\int_0^\infty
...
\begin{frame}
	...
	\begin{equation*}
		\frac{\mathrm{d}}{\mathrm{d}t}x(t)=a x(t)+b u_{\mathrm{p}}(t),
		\qquad y(t)=x(t),
		\qquad b\neq0 .
	\end{equation*}\pause%
	...
	\begin{equation*}
		\boxed{\frac{\mathrm{d}}{\mathrm{d}t}e(t)=a e(t)+b u(t).}
	\end{equation*}\pause%
...
\begin{frame}
	...
	\begin{equation*}
		\frac{\mathrm{d}}{\mathrm{d}t}\bm{e}(t)
		=\frac{\mathrm{d}}{\mathrm{d}t}\bm{x}(t)
	...
	\end{equation}
	subject to $\nicefrac{\mathrm{d}}{\mathrm{d}t}\bm{e}=\bm{A}\bm{e}+\bm{B}\Delta\bm{u}$.
\end{frame}
...
\begin{frame}
	...
	\end{equation}\vspace{-0.4cm}\pause%
...
\begin{frame}
	...
	\end{equation*}
	and $\bm{u}_{\mathrm{x},\mathrm{dq}}$ denotes an external induced-voltage contribution. \pause With
	\begin{equation*}
...
\begin{frame}
	...
	\begin{equation*}
		\frac{\mathrm{d}}{\mathrm{d}t}x(t)=u(t),
		\qquad x(0)=x_0>0,
...
\begin{frame}
	...
	For a current reference $\bm{i}_{\mathrm{s,dq}}^*$ and voltage deviation $\Delta u = \bm{u}_{\mathrm{s,dq}} - \bm{u}_{\mathrm{s,dq}}^*$ with $\bm{u}_{\mathrm{s,dq}}^*$ being the equilibrium voltage in the requested current set point, define
	\begin{equation*}
		\bm{e}_i=\bm{i}_{\mathrm{s,dq}}-\bm{i}_{\mathrm{s,dq}}^*,
		\qquad
		\ell_i(\bm{i}_{\mathrm{s,dq}},\bm{u}_{\mathrm{s,dq}})
		=\bm{e}_i^{\mathrm{T}}\bm{Q}_i\bm{e}_i
			+(\Delta u)^{\mathrm{T}}\bm{R}_u\Delta u .
	\end{equation*}\pause%
	...
	\end{equation*}
	subject to the current dynamics and the constraints $\mathcal{I}_\mathrm{s},\mathcal{U}_\mathrm{s}$. \pause%
	The value function is
	\begin{equation*}
...
\begin{frame}
	...
	\begin{equation*}
		\bm{u}_{\mathrm{unc}}^*
		=\bm{u}_{\mathrm{s,dq}}^*-\frac{1}{2}\bm{R}_u^{-1}\bm{p}.
	\end{equation*}\pause%
	...
	\begin{equation*}
		\boxed{\bm{u}_{\mathrm{s,dq}}^*
			...
			\left(\bm{u}_{\mathrm{s,dq}}^*-\frac{1}{2}\bm{R}_u^{-1}\bm{L}^{-\mathrm{T}}
			\nabla_{\bm{i}}v^*\right).}
	\end{equation*}\pause%
...
\begin{frame}
	...
	\begin{equation*}
		...
		\quad \forall t\geq0.
	\end{equation*}\pause%
	The finite-cost admissible current domain is the constraint-admissible viability kernel
	\begin{equation*}
...
\begin{frame}
	...
	\begin{equation}
		\begin{aligned}
			\min_{\bm{u}[k|k], \ldots, \bm{u}[k+N-1|k]}\quad
		...
		\label{eq:mpc_ocp}
	\end{equation}
	where $\mathcal{Z}=\{(\bm{x},\bm{u})\in\mathcal{X}\times\mathcal{U}\,|\,\bm{g}(\bm{x},\bm{u})\leq\bm{0}\}$ is \hl{the feasible operating area}, $\mathcal{X}_\mathrm{f}$ is the \hl{terminal state  set}, and $V_\mathrm{f}$ is the \hl{terminal cost} approximating the infinite-horizon value function $v^*$.  \pause  Apply only the first optimal input and shift the horizon (\hl{receding-horizon feedback}):
	\begin{equation}
		\bm{u}[k]=\bm{u}^*[k|k],
		\qquad
		k\leftarrow k+1 .
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[c]
	\frametitle{Receding horizon principle}
	\centering
	\begin{circuitikz}
		\def\hd{1.5}
		\def\vd{3}
...
\begin{frame}
	...
	\begin{equation}
		...
		\bm{y}[k]=\bm{C}\bm{x}[k].
	\end{equation}\pause%
	At time step $k$, define the \hl{quadratic finite-horizon tracking objective} (neglecting the terminal cost)
	\begin{equation}
		\mathcal{J}_{N}[k]
		=\sum_{j=0}^{N-1}
...
\begin{frame}
	...
	\begin{equation}
		...
		\end{bmatrix}.
	\end{equation}\pause%
	With $\bm{C}_{N}=\bm{I}_{N}\otimes \bm{C}$, where $\otimes$ denotes the Kronecker product, the output predictions are
	\begin{equation}
...
\begin{frame}
	...
	\begin{equation}
		\bm{R}_{N}[k]=\begin{bmatrix}\bm{r}[k|k]\T&\cdots&\bm{r}[k+N-1|k]\T\end{bmatrix}\T,
		\quad
...
\begin{frame}
	...
	\end{equation}\pause%
	Therefore, linear MPC with quadratic costs and linear constraints becomes the \hl{QP}
	\begin{equation}
		\boxed{
			\begin{aligned}
...
\begin{frame}
	...
	\end{equation}\pause%
	The objective is approximated quadratically around the current trajectory guess. \pause%
...
\begin{frame}
	...
	Every prediction step branches into $n_{\mathrm{s}}=|\mathbb{S}|$ successor actions. Hence, it applies
	\begin{equation*}
...
\begin{frame}
	...
	Exhaustive enumeration has worst-case complexity $\mathcal{O}(n_{\mathrm{s}}^{N})$. \pause%
	...
	\begin{itemize}
		\item simple lower bound (assuming non-negative costs): $\underline{\mathcal{J}}_{m}^{\mathrm{tail}}=0$,\pause%
		\item additional bounds due to constraints violations,\pause%
		\item branch ordering by promising stage costs to obtain a good incumbent early.\pause%
	\end{itemize}
...
\begin{frame}
	...
	\begin{equation*}
		\begin{array}{rcl}
			\text{short horizons}             & : & T_\mathrm{s}\text{ is much smaller than in modulator-based CCS},     \\[1mm]\pause%
			\text{parallel stage prediction}  & : & \text{FPGA acceleration},                                            \\[1mm]\pause%
			\text{branch and bound}           & : & \text{subtree pruning based on lower/upper bounds},                  \\[1mm]\pause%
			\text{move blocking / heuristics} & : & \text{reduced combinatorial search depth},                           \\[1mm]\pause%
			\text{explicit switching rules}   & : & \text{specialized approximations for }N=1 \text{ (e.g., DTC)}.\pause %
		\end{array}
...
\begin{frame}[c]
	...
	\begin{figure}
		...
		\caption{Transient torque response of a IM drive controlled by FCS-MPC for different prediction horizons $N_\mathrm{p}$ (modified from source: \href{https://ieeexplore.ieee.org/document/9830061}{10.1109/TPEL.2022.3190713}, \href{https://creativecommons.org/licenses/by/4.0/deed.en}{CC BY 4.0}).}
...
\begin{frame}
	...
	\end{equation}\pause%
	The \hl{discount factor} $\gamma$ weights future rewards: small $\gamma$ emphasizes immediate behavior, while $\gamma\rightarrow 1$ emphasizes long-term consequences. \pause For a drive-control tracking task, $\bm{z}[k]$ should include all variables such that the entire information describing closed-loop behavior is captured, e.g.,
	\begin{equation}
		\bm{z}[k]
		=\begin{bmatrix}
...
\begin{frame}
	...
	\end{equation}
	where $p_{\mathrm{c}}[k]\geq 0$ collects optional \hl{penalties} for constraint violations, e.g., overcurrent or inadmissible voltage usage.\pause%
...
\begin{frame}
	...
	\begin{equation}
		\boxed{
			...
			=\mathbb{E}\left[
				...
				\right].}
	\end{equation}\pause%
	Here, $(\bm{z}',\bm{u}')$ are the next state and action (at time $k+1$), respectively. \pause For a \hl{parameterized approximation} $q_{\bm{w}}\approx q^{*}$, the temporal-difference target is
	\begin{equation}
		y=r_{\mathrm{RL}}[k+1]
		+\gamma\max_{\bm{u}'}
	...
	\end{equation}\pause%
	Here, $\mathcal{D}$ is a \hl{replay (data) buffer} and $q_{\bar{\bm{w}}}$ is a slowly updated \hl{target network}. \pause  Hence, the loss is the mean-squared error of the estimator's output plugged into the Bellman optimality equation.
\end{frame}
		...
		\begin{column}[c]{0.45\textwidth}
			...
			\onslide<2->{
					\begin{tikzpicture}[
						...
						\node[process] (ann) {Estimator\\[1em]\includegraphics[width=1.25cm]{fig/lec05/ANN_symbol.pdf}};
...
\begin{frame}
	...
	\begin{itemize}
		\item enforce hard constraints by action masking, safety filters, or constrained training,\pause%
		\item include $\bm{s}[k-1]$ in $\bm{z}[k]$ when switching transitions are penalized,\pause%
		\item cover relevant operating points to limit extrapolation risk,\pause%
...
\begin{frame}
	...
	\begin{equation}
		...
		q_{\bm{w}}(\bm{z},\bm{u})
	\end{equation}
	at every sampling instant. \pause \hl{Actor-critic methods} avoid this inner optimization by learning a \hl{direct / explicit mapping} from state to actions:
	\begin{equation*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Differentiable predictive control in the OCP/MPC/RL landscape
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Differentiable predictive control in the OCP/MPC/RL landscape}
	The preceding methods can be interpreted as different approximations of the same ideal constrained OCP:
	\begin{equation*}
		\pi^{*}\in\argmin_{\pi\in\Pi_{\mathrm{adm}}}\mathcal{J}_{\infty}(\bm{x}_0;\pi).
	\end{equation*}
	\begin{center}
		\begin{tabular}{lll}
			\textbf{MPC} & $\rightarrow$ & solve a finite-horizon OCP online at each sampling step,                  \\[1mm]\pause%
			\textbf{RL}  & $\rightarrow$ & learns an explicit policy approximation from reward samples,              \\[1mm]\pause%
			\textbf{DPC} & $\rightarrow$ & train a parametrized policy through a differentiable rollout model.\pause %
		\end{tabular}
	\end{center}
	DPC uses the model-predictive cost structure, but avoids the online optimizer by learning
	\begin{equation*}
		\boxed{\bm{u}[k]=\pi_{\bm{\theta}}\bigl(\bm{x}[k],\bm{r}[k],\bm{\xi}[k]\bigr)}
	\end{equation*}
	offline over a distribution of operating scenarios $\bm{\xi}\sim\mathcal{D}$.\pause%
	\begin{block}{Key idea}
		If model, policy, costs, and soft constraint penalties are differentiable, the finite-horizon closed-loop loss can be minimized by gradient-based learning (i.e., self-supervised learning).
	\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{DPC: high-level visualization}
	\begin{figure}
		\includegraphics[width=0.8\textwidth]{fig/lec05/DPC_conceptual_overview.pdf}
		\caption{Conceptual methodology of differentiable predictive control (source: \href{https://www.sciencedirect.com/science/article/pii/S0959152422000981}{10.1016/j.jprocont.2022.06.001}, \href{https://creativecommons.org/licenses/by-nc-nd/4.0/}{CC BY-NC-ND 4.0}).}
		\label{fig:DPC_conceptual_overview}
	\end{figure}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Parametric finite-horizon control problem behind DPC
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Parametric finite-horizon control problem behind DPC}
	For one (rollout) \hl{scenario} $\bm{\xi}$, consider the finite-horizon constrained OCP
	\begin{equation}
		\begin{aligned}
			\min_{\bm{u}[0],\ldots,\bm{u}[N-1]}\quad
			 & \mathcal{J}_{N}(\bm{x}_0,\bm{\xi})
			=\sum_{k=0}^{N-1}\ell\bigl(\bm{x}[k],\bm{u}[k],\bm{r}[k];\bm{\xi}\bigr)
			+\ell_{\mathrm{f}}\bigl(\bm{x}[N],\bm{r}[N];\bm{\xi}\bigr)                  \\
			\mathrm{s.t.}\quad
			 & \bm{x}[0]=\bm{x}_0(\bm{\xi}),                                            \\
			 & \bm{x}[k+1]=\bm{f}_{\mathrm{d}}\bigl(\bm{x}[k],\bm{u}[k];\bm{\xi}\bigr), \\
			 & \bm{g}_{\mathrm{d}}\bigl(\bm{x}[k],\bm{u}[k];\bm{\xi}\bigr)\leq\bm{0}.
		\end{aligned}
	\end{equation}\pause%
	DPC replaces this implicit optimization problem (as considered in MPC) by an \hl{explicit policy approximation} (as resulting from RL):
	\begin{equation}
		\boxed{\bm{u}[0],\ldots,\bm{u}[N-1]\approx \pi_{\bm{\theta}}(\bm{x}[0],\bm{r}[0],\bm{\xi}), \ldots, \pi_{\bm{\theta}}(\bm{x}[N-1],\bm{r}[N-1],\bm{\xi}).}
	\end{equation}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Differentiable closed-loop rollout}
	For fixed $\bm{\theta}$ and $\bm{\xi}$, DPC closes the loop over the prediction horizon:
	\begin{equation}
		\bm{x}[0]=\bm{x}_0(\bm{\xi}),\qquad
		\bm{u}_{\bm{\theta}}[k]=\pi_{\bm{\theta}}\bigl(\bm{x}[k],\bm{r}[k],\bm{\xi}\bigr),
	\end{equation}
	\begin{equation}
		\bm{x}[k+1]=\bm{f}_{\mathrm{d}}\bigl(\bm{x}[k],\bm{u}_{\bm{\theta}}[k];\bm{\xi}\bigr),
		\qquad k=0,\ldots,N-1.
	\end{equation}\pause%
	The \hl{differentiable DPC loss} is
	\begin{equation}
		\mathcal{L}_{N}(\bm{\theta};\bm{\xi})
		=\sum_{k=0}^{N-1}\ell_{\mathrm{DPC}}[k]+\ell_{\mathrm{f}}\bigl(\bm{x}[N],\bm{r}[N];\bm{\xi}\bigr),
	\end{equation}\pause%
	with a \hl{soft-constrained stage loss}, e.g.,
	\begin{equation}
		\ell_{\mathrm{DPC}}[k]
		=\ell\bigl(\bm{x}[k],\bm{u}_{\bm{\theta}}[k],\bm{r}[k];\bm{\xi}\bigr)
		+\left\|\left[\bm{g}_{\mathrm{d}}\bigl(\bm{x}[k],\bm{u}_{\bm{\theta}}[k];\bm{\xi}\bigr)\right]_{+}\right\|_{\bm{Q}_{\mathrm{g}}}^{2}.
	\end{equation}\pause%
	Here $[\cdot]_+$ denotes the componentwise positive part, i.e., only constraint violations are penalized.
\end{frame}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% DPC training objective: optimize a policy over scenarios
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{DPC training objective: optimize a policy over scenarios}
	Instead of solving one OCP online, \hl{DPC trains one policy over a scenario distribution} (rollouts):
	\begin{equation*}
		\boxed{
			\bm{\theta}^{*}\in\argmin_{\bm{\theta}}
			\mathbb{E}_{\bm{\xi}\sim\mathcal{D}}
			\left[\mathcal{L}_{N}(\bm{\theta};\bm{\xi})\right].}
	\end{equation*}\pause%
	In practice, the expectation is approximated by a mini-batch $\mathcal{B}$:
	\begin{equation*}
		\hat{\mathcal{L}}_{\mathcal{B}}(\bm{\theta})
		=\frac{1}{|\mathcal{B}|}\sum_{\bm{\xi}\in\mathcal{B}}\mathcal{L}_{N}(\bm{\theta};\bm{\xi}).
	\end{equation*}\pause%
	Implementing the complete \hl{closed-loop computation graph in differentiable programming framework} (e.g., JAX or PyTorch),
	\begin{equation*}
		\nabla_{\bm{\theta}}\hat{\mathcal{L}}_{\mathcal{B}}(\bm{\theta})
	\end{equation*}
	can be computed by automatic differentiation.\pause%
	\begin{block}{Interpretation}
		The DPC loss acts like a white-box finite-horizon value estimate: it is generated by a known model and directly differentiated with respect to the policy parameters.
	\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Backpropagation through predictive dynamics
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Backpropagation through predictive dynamics}
	The \hl{sensitivity} of the predicted trajectory follows from the chain rule. Define
	\begin{equation*}
		\bm{S}_{x}[k]=\frac{\partial \bm{x}_{\bm{\theta}}[k]}{\partial \bm{\theta}},
		\qquad
		\bm{S}_{x}[0]=\bm{0}.
	\end{equation*}\pause%
	With
	\begin{equation*}
		\bm{F}_{x}[k]=\frac{\partial \bm{f}_{\mathrm{d}}}{\partial\bm{x}}[k],\quad
		\bm{F}_{u}[k]=\frac{\partial \bm{f}_{\mathrm{d}}}{\partial\bm{u}}[k],\quad
		\bm{\Pi}_{x}[k]=\frac{\partial \pi_{\bm{\theta}}}{\partial\bm{x}}[k],\quad
		\bm{\Pi}_{\theta}[k]=\frac{\partial \pi_{\bm{\theta}}}{\partial\bm{\theta}}[k],
	\end{equation*}\pause%
	one obtains the forward sensitivity recursion
	\begin{equation*}
		\boxed{
			\bm{S}_{x}[k+1]
			=\bm{F}_{x}[k]\bm{S}_{x}[k]
			+\bm{F}_{u}[k]\left(\bm{\Pi}_{x}[k]\bm{S}_{x}[k]+\bm{\Pi}_{\theta}[k]\right).}
	\end{equation*}
	Equivalently, modern implementations compute the same gradient by reverse-mode automatic differentiation.\pause%
	\begin{block}{Relation to optimal control}
		The backward pass corresponds to a discrete adjoint calculation; in the continuous-time limit this links to Pontryagin-type adjoint equations.
	\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Basic DPC training loop
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Basic DPC training loop}
	A basic self-supervised DPC training loop is:
	\begin{enumerate}
		\item sample a mini-batch of scenarios $\mathcal{B}=\{\bm{\xi}_{1},\ldots,\bm{\xi}_{B}\}$,
		\item roll out the closed-loop model with $\bm{u}_{\bm{\theta}}[k]=\pi_{\bm{\theta}}(\bm{x}[k],\bm{r}[k],\bm{\xi})$,
		\item evaluate tracking, input-effort, terminal, and constraint-violation terms,
		\item compute $\nabla_{\bm{\theta}}\hat{\mathcal{L}}_{\mathcal{B}}(\bm{\theta})$ by automatic differentiation,
		\item update the policy, e.g., $\bm{\theta}\leftarrow\bm{\theta}-\alpha\nabla_{\bm{\theta}}\hat{\mathcal{L}}_{\mathcal{B}}(\bm{\theta})$.
	\end{enumerate}
	\begin{center}
		\begin{tikzpicture}[>=Latex,node distance=7mm and 5mm,
				box/.style={draw,rounded corners,minimum height=8mm,minimum width=22mm,align=center,font=\scriptsize}]
			\node[box] (scen) {scenarios\\$\bm{\xi}\sim\mathcal{D}$};
			\node[box,right=of scen] (pol) {policy\\$\pi_{\bm{\theta}}$};
			\node[box,right=of pol] (model) {model\\$\bm{f}_{\mathrm{d}}$};
			\node[box,right=of model] (loss) {DPC loss\\$\mathcal{L}_{N}$};
			\draw[-Latex] (scen) -- (pol);
			\draw[-Latex] (pol) -- (model);
			\draw[-Latex] (model) -- (loss);
			\draw[-Latex,dashed] (loss.north) .. controls +(0,0.8) and +(0,0.8) .. node[above,font=\scriptsize] {gradient} (pol.north);
		\end{tikzpicture}
	\end{center}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Data-driven DPC training: model system identification + policy learning}
	\begin{figure}
		\includegraphics[width=0.39\textwidth]{fig/lec05/DPC_SysID.pdf}\hspace{1cm}
		\includegraphics[width=0.39\textwidth]{fig/lec05/DPC_Policy_Training.pdf}
		\caption{Learning a model from data (left) to training a policy (right) (source: \href{https://www.sciencedirect.com/science/article/pii/S0959152422000981}{10.1016/j.jprocont.2022.06.001}, \href{https://creativecommons.org/licenses/by-nc-nd/4.0/}{CC BY-NC-ND 4.0}).}
		\label{fig:DPC_sysid_policy_training}
	\end{figure}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Practical strengths and caveats}
	\begin{columns}[T,totalwidth=\textwidth]
		\begin{column}{0.46\textwidth}
			\begin{block}{Strengths}
				\begin{itemize}
					\item shifts computation offline,
					\item enables fast online policy evaluation,
					\item uses model-predictive tracking and constraint penalties,
					\item does not require expert MPC demonstrations.
				\end{itemize}
			\end{block}\pause%
		\end{column}
		...
		\begin{column}{0.46\textwidth}
			\begin{block}{Caveats}
				\begin{itemize}
					\item gradients require differentiability,
					\item constraint satisfaction is often soft unless additional safety layers are used,
					\item training quality depends on scenario coverage and model accuracy,
					\item no exact online optimality certificate.
				\end{itemize}
			\end{block}\pause%
		\end{column}
	\end{columns}
	\vspace{2mm}
	\begin{block}{Software and further reading}
		Example DPC/L2C implementations are available in \href{https://github.com/pnnl/neuromancer}{NeuroMANCER} and the \href{https://sciml-l4dc2026.github.io/SciML-L4DC2026}{SciML L2C tutorial material}.
	\end{block}
\end{frame}

...
\begin{frame}
    ...
    \begin{equation}\label{eq:system_det_dyn}
        \begin{split}
            \frac{\mathrm{d}}{\mathrm{d}t}\bm{x}(t) & = \bm{f}(\bm{x}(t), \bm{u}(t))\, , \quad \bm{x}(t_0)=\bm{x}_0\, , \\
            \bm{y}(t)                               & =\bm{g}\left(\bm{x}(t),\bm{u}(t)\right)
        \end{split}
    \end{equation}
    with state $\bm{x}(t)\in \mathbb{R}^n$, input $\bm{u}(t)\in \mathbb{R}^m$, output $\bm{y}(t)\in \mathbb{R}^p$, time $t\in \mathbb{R}$, and nonlinear functions $\bm{f}(\cdot): \mathbb{R}^n \times \mathbb{R}^m  \to \mathbb{R}^n$ and $\bm{g}(\cdot): \mathbb{R}^n \times \mathbb{R}^m  \to \mathbb{R}^p$. \pause For each discrete time step $k\in \mathbb{N}_0$ with step size $T_\mathrm{s}$, the state and output at time $t=(k+1)T_\mathrm{s}$ are approximated by the \hl{ Runge-Kutta (RK) method} as
    \begin{equation}
        \begin{split}
            \bm{x}[k+1] & = \bm{x}[k] + T_\mathrm{s}\sum_{j=1}^{s}b_j\bm{h}_j\, , \quad \bm{h}_j  = \bm{f}\left(\bm{x}[k]+T_\mathrm{s} \sum_{l=1}^{s}\alpha_{j,l}\cdot\bm{h}_l, \,\,\bm{u}(t_k+c_j T_\mathrm{s})\right), \\
...
\begin{frame}
    ...
    The RK-method \eqref{eq:system_det_dyn_RK_explizit} is called \hl{explicit} if $\alpha_{j,l}=0$ for all $l\geq j$, which allows to compute the stage derivatives sequentially without solving implicit equations.\\[1em]\pause
    ...
    \begin{equation}
        \begin{split}
            b_1           = b_4 = \frac{1}{6}, \quad b_2 = b_3 = \frac{1}{3},                                                                           \qquad
            c_1           = 0, \quad c_2 = c_3 = \frac{1}{2}, \quad c_4 = 1,                                                                             \\
            \alpha_{2,1}  = \alpha_{3,2} = \frac{1}{2}, \quad \alpha_{4,3} = 1, \quad \alpha_{j,l} = 0 \text{ for } \{i,j\}\notin \{(2,1),(3,2),(4,1)\}. \\
        \end{split}
        \label{eq:RK4_coeff}
    \end{equation}
\end{frame}
...
\begin{frame}
    ...
    A RK method is called \hl{implicit} if $\alpha_{j,l}\neq 0$ for some $l\geq j$, which requires solving a nonlinear equation systems in each step. Hence, implicit methods are typically more computationally expensive per step but can offer better stability properties.\\\pause
    ...
    \end{equation}
    \\\pause
    \textbf{Implicit classical Runge-Kutta (IRK4)}: There are multiple implicit RK methods of order four, e.g., the \href{https://en.wikipedia.org/wiki/Gauss-Legendre_method}{Gauss-Legendre method} with  coefficients
    \[
        \begin{array}{c@{\qquad\qquad}c}
            %
            \text{\textit{Butcher tableau (general 4-stage)}}
             &
            \text{\textit{Gauss-Legendre coefficients}}
            \\[1.2ex]
            %
            \begin{array}{c|cccc}
                c_1
...
\begin{frame}
    ...
    \end{equation}
    with the constant matrices $\bm{A}\in \mathbb{R}^{n\times n}$, $\bm{B}\in \mathbb{R}^{n\times m}$, $\bm{C}\in \mathbb{R}^{p\times n}$, and $\bm{D}\in \mathbb{R}^{p\times m}$. \pause Assuming zero-order hold (ZOH) for the input $\bm{u}(t)$, the \hl{exact discrete-time representation} is given by
    \begin{equation}
        \begin{split}
    ...
    \end{equation}\pause
    The last identity only holds if $\bm{A}$ is invertible. The \hl{matrix exponential} $e^{\bm{A}T_\mathrm{s}}$ can be numerically expensive to compute and, therefore, is typically precomputed offline.
\end{frame}
...
\begin{frame}
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}\pause
    This can be found by either a first-order Taylor series expansion of the matrix exponential \eqref{eq:exact_discretization_matrices} or by applying the RK1 method \eqref{eq:RK1} directly to the LTI system. \pause Likewise, the \hl{implicit Euler method (IRK1)} for the LTI system is given by
    \begin{equation}
        \begin{split}
            \bm{x}[k+1]  = \bm{x}[k] + T_\mathrm{s}(\bm{A}\bm{x}[k+1] + \bm{B}\bm{u}[k])\quad             \bm{y}[k]    = \bm{C}\bm{x}[k] + \bm{D}\bm{u}[k]
        \end{split}
    \end{equation}\pause
    leading to
    \begin{equation}
        \begin{split}
            \bm{x}[k+1]  = (\bm{I} - T_\mathrm{s}\bm{A})^{-1}\bm{x}[k] + (\bm{I} - T_\mathrm{s}\bm{A})^{-1}T_\mathrm{s}\bm{B}\bm{u}[k], \quad
            \bm{y}[k]    = \bm{C}\bm{x}[k] + \bm{D}\bm{u}[k].
        \end{split}
    \end{equation}

...
COLOR_R = (213 / 255, 94 / 255, 0 / 255)
COLOR_B = (0 / 255, 114 / 255, 178 / 255)
COLOR_G = (0 / 255, 158 / 255, 115 / 255)

OUTPUT_GIF = Path(__file__).with_name("Clarke_Park.gif")
OUTPUT_PREVIEW = Path(__file__).with_name("Clarke_Park_preview.png")
...
park_transforms = [
    park_transform(i_a[i], i_b[i], i_c[i], angles[i] + deg_dq) for i in range(n_frames)
]
...
def update_plot(i):
    ...
    axes[0, 0].arrow(
        (i_a[i] + i_b[i] * np.cos(2 / 3 * np.pi)) * 2 / 3,
        i_b[i] * np.sin(2 / 3 * np.pi) * 2 / 3,
        i_c[i] * np.cos(4 / 3 * np.pi) * 2 / 3,
    ...
    axes[0, 0].text(
        -1 / 2 - 0.18, np.sqrt(3) / 2 + 0.025, "b", fontsize=12, va="center"
    )
    axes[0, 0].text(
        -1 / 2 - 0.18, -np.sqrt(3) / 2 - 0.05, "c", fontsize=12, va="center"
    )
    ...
    axes[0, 1].plot(angles, i_a, color=COLOR_R)
    axes[0, 1].plot(angles, i_b, color=COLOR_B)
    axes[0, 1].plot(angles, i_c, color=COLOR_G)
    ...
    axes[0, 1].scatter(theta, i_a[i], color=COLOR_R, marker="o")
    axes[0, 1].scatter(theta, i_b[i], color=COLOR_B, marker="o")
    axes[0, 1].scatter(theta, i_c[i], color=COLOR_G, marker="o")
    ...
    axes[1, 0].text(1.1, 0, r"$\upalpha$", fontsize=12, va="center")
    axes[1, 0].text(0, 1.15, r"$\upbeta$", fontsize=12, va="center", ha="center")
    circle = plt.Circle((0, 0), 1, color="black", fill=False, lw=1)
    ...
    axes[1, 1].plot(angles, clarke_transform(i_a[:], i_b[:], i_c[:])[0], color=COLOR_R)
    axes[1, 1].plot(angles, clarke_transform(i_a[:], i_b[:], i_c[:])[1], color=COLOR_B)
    axes[1, 1].set_xlim(0, 2 * np.pi)
    axes[1, 1].set_ylim(-1.1, 1.1)
    axes[1, 1].set_ylabel(r"$x_\mathrm{abc}$")
    axes[1, 1].set_title(r"$\upalpha\upbeta$ signals")
    ...
    axes[1, 1].scatter(
        theta, clarke_transform(i_a[i], i_b[i], i_c[i])[0], color=COLOR_R, marker="o"
    )
    axes[1, 1].scatter(
        theta, clarke_transform(i_a[i], i_b[i], i_c[i])[1], color=COLOR_B, marker="o"
    )
    ...
    axes[2, 0].arrow(
        -np.cos(theta + deg_dq),
        -np.sin(theta + deg_dq),
        2 * np.cos(theta + deg_dq),
        2 * np.sin(theta + deg_dq),
        head_width=0.05,
        head_length=ahl,
        fc="k",
        ec="k",
    ...
    axes[2, 0].arrow(
        -np.cos(theta + deg_dq + np.pi / 2),
        -np.sin(theta + deg_dq + np.pi / 2),
        2 * np.cos(theta + deg_dq + np.pi / 2),
        2 * np.sin(theta + deg_dq + np.pi / 2),
        head_width=0.05,
        head_length=ahl,
        fc="k",
        ec="k",
    ...
    axes[2, 0].text(
        np.cos(theta + deg_dq) * 1.15,
        np.sin(theta + deg_dq) * 1.15,
        "d",
        fontsize=12,
        va="center",
        ha="center",
    ...
    axes[2, 0].text(
        np.cos(theta + deg_dq + np.pi / 2) * 1.15,
        np.sin(theta + deg_dq + np.pi / 2) * 1.15,
        "q",
        fontsize=12,
        va="center",
        ha="center",
    ...
    axes[2, 1].plot(angles, ids, color=COLOR_B)
    axes[2, 1].plot(angles, iqs, color=COLOR_R)
...
update_plot(20)
fig.savefig(OUTPUT_PREVIEW, dpi=330, bbox_inches="tight")
plt.show()
